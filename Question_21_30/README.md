...
--補足--
例としてはちょっとよろしくない。RGB全部を一緒くたにまとめて平坦化してしまうと色味が大きく変わってしまったりする。  
グレイスケールで通常は使用される手法。
よくあるのは空などが大きく写っている画像で、適当にN分割などで減色をすると、以外と明るさのレンジが広くて等高線のようなパターンが現れてしまう。
ここで、ヒストグラム平坦化を用いて大きな面積を有する領域には細かく色を設定してやると緩やかなグラデーションは綺麗に表示されるようになる。
その分、小さい物体などで色が少し変になります。
